...
# Training Parameters
scenario = 'configs/all_skills_v2.cfg'
...
def train_heirarchical_model():
    ...
    model = HDQNModel(sub_models=models, skill_frame_skip=2, resolution=doom.get_processed_state(depth_radius, depth_contrast).shape[-2:], nb_frames=learn_param['nb_frames'], actions=doom.actions, depth_radius=depth_radius, depth_contrast=depth_contrast)
    agent = RLAgent(model, **learn_param)
